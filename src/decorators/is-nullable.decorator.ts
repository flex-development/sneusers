...
import { isNull, type ObjectUnknown } from '@flex-development/tutils'
import {
  getMetadataStorage,
  ValidationTypes,
...
/**
 * Ignores all validators if a property value is `null`.
 *
 * @decorator
 *
  ...
  return function (obj: object, name: string | symbol): void {
    getMetadataStorage().addValidationMetadata(
      new ValidationMetadata({
        constraints: [(obj: ObjectUnknown) => !isNull(obj[name])],
